...
const CommentCard = ({ index, leftVal, commentData}) => {
    ...
    const handleReply = () => {
        if(!access_token){
            return toast.error("Please sign in to reply to comments")
        ...
        <div className="w-full" style={{paddingLeft: `${leftVal * 10}px`}}>
            ...
            <div className="my-5 p-6 rounded-md border border-grey">
                <div className="flex gap-2 items-center mb-8">
                    ...
                    <p className="line-clamp-2 capitalize">{fullname}</p>
                ...
                <div className="flex gap-5 items-center mt-5">

                    {
                        ...
                        <button
                        onClick={hideReplies} 
                        className="text-dark-grey p-2 px-3 bg-grey/50 hover:bg-grey/80 rounded-md flex items-center gap-2">
                             <i className="fi fi-rs-comment-dots"></i>Hide Replies
                        </button> 
                        : 
                        <button className="text-dark-grey p-2 px-3 bg-grey/50 hover:bg-grey/80 rounded-md flex items-center gap-2" onClick={loadReplies}> 
                            <i className="fi fi-rs-comment-dots"/>
                            {
                                children.length != 1 ? `${children.length} Replies` : `${children.length} Reply` 
                            }
                        </button>
                    }
...

                    {
                        ...
                        <button className="p-2 px-3 rounded-md ml-auto hover:text-red flex items center" onClick={deleteComment}><i className="fi fi-rr-trash pointer-events-none"></i></button>
